...
### Deterministic voice trigger + grounded-explanation + video controls — all shipped 2026-06-26 through 2026-06-30
...
- [x] VoiceLoop 6-way intercept branch — `parseCommand` reordered above `initMediaSource` (silent-video path returns before pipeline opens; `finally` still fires → `setStatus("idle")`, no explicit teardown); `stagedAssetRef = useRef<Asset | null>(null)` tracks currently-staged asset for bare `explain` lookup in commit 2b; `SHOW_ACKS`/`CLEAR_ACKS` canned-ack rotations; silent early-return for `show + video + !explain`; explain=true falls through to chat fetch. Smoke: image-show ack / clear-ack / show+explain / normal ✅ ears-proven. **Silent-video: EARS-PROVEN 2026-06-27** (cold dev-server restart + fresh tab; stale Next fetch-cache was root cause of missing-row confusion) — `68ee4ec`
- [x] Grounded-explanation 2b — staged asset `description` folded into the user-turn on explain paths (`[On screen: …]` prefix injected client-side, NOT in system blocks — would bust prompt cache); bare-explain uses `stagedAssetRef.current.description`; free-form stays ungrounded by design; Mask riffs not recites — `ec80b83`
- [x] Stage video unmuted + `playsInline` — removed hardcoded `muted` from Stage.tsx; autoplay works post-arm via page engagement, no prewarm needed; ears-proven on both push-to-talk and wake-word paths — `4d2526a`
- [x] Video Pause/Resume control + stop looping — `loop` dropped (plays once, parks on last frame via `onEnded→paused`); Pause/Resume button in page.tsx (video-only, `fixed bottom-6 left-8 z-20`); `paused` state owned by page.tsx, resets on new asset id; VoiceLoop bottom-cluster `pointer-events-none` fix unblocked the button (full-width bar was intercepting clicks on transparent regions — confirmed via `elementFromPoint`) — `fb9e8e9`
...
**Banked lessons:** (a) stale-assetsRef + Next fetch-cache — VoiceLoop fetches `/api/assets` once on mount; DB rows added after dev-server start are invisible until cold restart (`lsof -ti:3000|xargs kill -9` + `rm -rf .next` + `npm run dev` + fresh tab). (b) fabricated-cause discipline — Next-cache staleness was initially misdiagnosed as a phantom RLS policy; simplest cause fitting all evidence wins. (c) overlay-click interception — `fixed inset-x-0` full-width bars eat pointer events on transparent regions; `pointer-events-none` + `pointer-events-auto` on children is the fix.

**Known issue — /api/assets fetch-cache staleness:** an asset uploaded while the server is running may not appear in VoiceLoop until cache clears (cold server restart + fresh tab). Fix pending: add `export const revalidate = 0` (or `force-dynamic`) to `app/api/assets/route.ts` — audit before editing.

**Next (recommended order):** (1) neuter `<stage>` in `personality.ts` + re-snapshot [scoped unlock]; (2) `/api/assets` fetch-cache fix; (3) hardware gate when DJI Mic + R400 arrive. Then: vocabulary-robustness, interrupt/clicker slice (Pause is its on-screen half), admin EDIT, autoplay timing, streaming STT, wake word V2.
